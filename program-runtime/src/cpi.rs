...
struct SolAccountMetaV2 {
    pub runtime_account_addr: u64,
    pub is_writable: bool,
    pub is_signer: bool,
}

impl SolAccountMetaV2 {
    /// Return the wrapped `RuntimeAccount` for this `SolAccountMetaV2`.
    #[inline(always)]
    pub fn runtime_account(&self) -> *const RuntimeAccount {
        // SAFETY: The `raw` pointer is guaranteed to be valid.
        self.runtime_account_addr as *const RuntimeAccount
    }
    ...
    pub fn address(&self) -> &Pubkey {
        // SAFETY: The `raw` pointer is guaranteed to be valid.
        unsafe { &(*self.runtime_account()).address }
    }
    ...
    pub fn data_len(&self) -> usize {
        // SAFETY: The `raw` pointer is guaranteed to be valid.
        unsafe { (*self.runtime_account()).data_len as usize }
    }
}
...
impl<'a> CallerAccount<'a> {
    ...
    fn from_account_meta_v2(
        ...
        let data_addr = (account_meta.runtime_account_addr as *const RuntimeAccount as u64)
            .saturating_add(std::mem::size_of::<RuntimeAccount>() as u64);

        if syscall_parameter_address_restrictions {
            check_account_info_pointer(
                invoke_context,
                account_meta.runtime_account_addr,
...
pub fn translate_accounts_v2<'a>(
    ...
    translate_account_infos(
        account_metas_addr,
        account_metas_len,
        |account_meta: &SolAccountMetaV2| account_meta.address() as *const Pubkey as u64,
        invoke_context,
        memory_mapping,
        check_aligned,
...
pub fn translate_instruction_v2(
    ...
    // While the `SolAccountMetaV2` struct is 10 bytes (8 for account_addr, 1 for is_signer, and
    ...
    for account_meta in account_metas {
        ...
        let runtime_account = translate_type::<RuntimeAccount>(
            memory_mapping,
            account_meta.runtime_account() as u64,
            false,
        )?;

        accounts.push(AccountMeta {
            pubkey: runtime_account.address,
            is_signer: account_meta.is_signer,
            is_writable: account_meta.is_writable,
        });
    }
